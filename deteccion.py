...
def iniciar_deteccion(guardar_malla=False):
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # coordenadas dentro de los límites de la imagen del reconocimiento
                x_min = max(x_min, 0)
                x_max = min(x_max, iw)
                y_min = max(y_min, 0)
                y_max = min(y_max, ih)
                ...
                if blink_count >= blink_threshold:
                    ...
                    img_path = os.path.join('capturas', f'captura_{timestamp}.png')
                    cv2.imwrite(img_path, rostro)
                    blink_count = 0

                    # Cerrar la cámara después de guardar la imagen
                    cap.release()
                    cv2.destroyAllWindows()
                    return img_path

                # Muestra la malla facial
                if guardar_malla:
                    ...
                    cv2.imshow('Detección de malla facial', frame_with_mesh)
